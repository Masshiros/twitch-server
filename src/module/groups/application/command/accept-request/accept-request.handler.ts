...
import { AcceptRequestCommand } from "./accept-request.command"
...
export class AcceptRequestHandler {
  ...
  async execute(command: AcceptRequestCommand) {
    ...
    try {
      ...
      if (request.status === EGroupRequestStatus.PENDING) {
        request.status = EGroupRequestStatus.APPROVED
        ...
        await Promise.all([
          this.groupRepository.updateRequest(request),
